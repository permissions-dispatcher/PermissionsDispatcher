...
class SystemAlertWindowHelper : SensitivePermissionInterface {
    ...
    override fun addHasSelfPermissionsCondition(builder: FunSpec.Builder, activityParam: String, permissionField: String) {
        builder.beginControlFlow("if (%T.hasSelfPermissions(%L, *%N) || %T.canDrawOverlays(%L))", PERMISSION_UTILS, activityParam, permissionField, SETTINGS, activityParam)
    }
    ...
    override fun addRequestPermissionsStatement(builder: FunSpec.Builder, targetParam: String, activityParam: String, requestCodeField: String) {
        builder.addStatement("val intent = %T(%T.ACTION_MANAGE_OVERLAY_PERMISSION, %T.parse(\"package:\" + %L.getPackageName()))", INTENT, SETTINGS, URI, activityParam)
