...
abstract class KotlinBaseProcessorUnit(val messager: Messager) : KtProcessorUnit {
    ...
    private fun addWithPermissionCheckBody(builder: FunSpec.Builder, needsMethod: ExecutableElement, rpe: RuntimePermissionsElement) {
        ...
        val activityParam = if (!targetIsActivity) "activity" else getActivityName()
    ...
    private fun createPermissionRequestClass(rpe: RuntimePermissionsElement, needsMethod: ExecutableElement): TypeSpec {
        ...
        val requestCodeField = requestCodeFieldName(needsMethod)
        val targetIsActivity = rpe.isSubtypeOf("${ACTIVITY.packageName}.${ACTIVITY.simpleName}")
        if (!targetIsActivity && ADD_WITH_CHECK_BODY_MAP[needsMethod.getAnnotation(NeedsPermission::class.java).value[0]] != null) {
            proceedFun
