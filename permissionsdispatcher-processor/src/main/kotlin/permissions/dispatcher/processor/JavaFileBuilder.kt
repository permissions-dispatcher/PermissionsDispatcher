...
import permissions.dispatcher.processor.Utils.getPermissionFieldName
import permissions.dispatcher.processor.Utils.getRequestCodeFieldName
...
object JavaFileBuilder {
    ...
    private fun createNeedsPermissionMethods(clazz: RuntimePermissionsAnnotatedElement): List<MethodSpec> {
        ...
        for (element in elements) {
            ...
            val activity = clazz.classType.getActivity()
    ...
    private fun createMethodWithCheck(activity: String, target: ClassName, needsPermission: ExecutableElement, showsRationale: ExecutableElement?): MethodSpec {
        ...
        val value = getPermissionFieldName(needsPermission.getSimpleName().toString())
        val methodBuilder = MethodSpec.methodBuilder(methodName + METHOD_SUFFIX).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(Void.TYPE).addParameter(target, "target").beginControlFlow("if ($T.hasSelfPermissions($N, $N))", PERMISSION_UTILS, activity, value).addStatement("target.$N()", methodName).nextControlFlow("else")
        if (showsRationale != null) {
            methodBuilder.beginControlFlow("if ($T.shouldShowRequestPermissionRationale($N, $N))", PERMISSION_UTILS, activity, value).addStatement("target.$N()", showsRationale.getSimpleName()).endControlFlow()
        }
        return methodBuilder.addStatement("$T.requestPermissions($N, $N, $N)", ACTIVITY_COMPAT, activity, value, getRequestCodeFieldName(methodName)).endControlFlow().build()
    }
    ...
    private fun createOnRequestPermissionsResult(target: ClassName, methods: List<ExecutableElement>): MethodSpec {
        ...
        for (method in methods) {
            ...
            methodBuilder.addCode("case $N:\n", getRequestCodeFieldName(methodName)).beginControlFlow("if ($T.verifyPermissions(grantResults))", PERMISSION_UTILS).addStatement("target.$N()", methodName).endControlFlow().addStatement("break")
        }
    ...
    private fun createFields(elements: List<ExecutableElement>): List<FieldSpec> {
        ...
        var index = 0
        for (element in elements) {
            val requestCodeFieldName = getRequestCodeFieldName(element.getSimpleName().toString())
            fieldSpecs.add(createRequestCodeField(requestCodeFieldName, index++))
            val permissionFieldName = getPermissionFieldName(element.getSimpleName().toString())
    ...
    private fun createRequestCodeField(name: String, index: Int): FieldSpec {
        return FieldSpec.builder(Integer.TYPE, name, Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL).initializer("$L", index).build()
    }

    private fun createPermissionField(name: String, vararg value: String): FieldSpec {
        return FieldSpec.builder(javaClass<Array<String>>(), name, Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL).initializer("$N", "new String[]" + Utils.toString(*value)).build()
    }
    ...
    fun createJavaFile(element: RuntimePermissionsAnnotatedElement): JavaFile {
        ...
        val clazz = TypeSpec.classBuilder(element.getDispatcherClassName()).addModifiers(Modifier.PUBLIC, Modifier.FINAL).addFields(createFields(permissionsMethods)).addMethod(createConstructor()).addMethods(createNeedsPermissionMethods(element)).addMethods(createNeedsPermissionsMethods(element)).addMethod(createOnRequestPermissionsResult(element.getClassName(), permissionsMethods)).build()
